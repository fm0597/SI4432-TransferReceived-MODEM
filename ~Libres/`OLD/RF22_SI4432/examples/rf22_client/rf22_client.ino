...
float OperatingFreq     = 435.0;                                //Рабочая_частота_передатчика_в_мГц
      ...
      if (RF22_Si4432.recv(Data, &len))
        {
         ...
         RF22LastRssi = RF22_Si4432.lastRssi();
         return true;
        }
      else return false;
    }
  ...
  if (!FLERRInitRHRF22) { 
    if (RF22_Si4432.waitAvailableTimeout(RF_TIME_OUT)) {
      ...
      if (RF22_Si4432.recv(buf, &len)) {
        ...
        Serial.print("RSSI: ");
        Serial.println(RF22_Si4432.lastRssi(), DEC);
        #endif
        //------------------------------------------------------
        // Send a reply
...
void setup() {
  ...
  if (RF22_Si4432.init() == false) FLERRInitRHRF22 = true;    //Флаг_ошибки_инициализации_модуля_SI4432
    ...
    //RF22_Si4432.setFrequency(434.0, 0.1);
    //RF22_Si4432.setModemConfig(RF22_Si4432.GFSK_Rb19_2Fd9_6);
    RF22_Si4432.setModemConfig(RF22_Si4432.FSK_Rb2Fd5);
    //RF22_Si4432.setTxPower(RH_RF22_TXPOW_20DBM);
  }
 //-------------------------------------------------------------
...
void loop() {
  ...
  //------------------------------------------------------------  
  DataSendSI4432(buf, sizeof(buf), OperatingFreq, RH_RF22_TXPOW_20DBM, PACAGE_LOOP_TX, RF_TIME_OUT);
  //------------------------------------------------------------
